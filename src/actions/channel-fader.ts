...
@action({ UUID: "com.wheatlandchurch.x32-mixer.fader" })
export class ChannelFaderAction extends SingletonAction<Settings> {
  ...
  private async updateButtonTitle(action: any): Promise<void> {
    ...
    const title = `Ch${action.getSettings().channel || '?'}\\n${dBLevel}dB`;
    await action.setTitle(title);
  }

  private async updateAllButtonTitles(): Promise<void> {
    const instances = streamDeck.actions.getActionById("com.wheatlandchurch.x32-mixer.fader");
    if (instances) {
